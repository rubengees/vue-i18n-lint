...
- **`removeUnused` command**: Detects unused keys via `processFiles` (with `missingKeys` and `dynamicKeys` severity off), then removes them from global locale files in-place. Uses `confbox` stringifiers (JSON/JSONC/JSON5/YAML) with a two-space indent. Handles nested key deletion and cleans up empty parents. `--dry-run` flag skips writes and only prints the count. Array elements are reindexed on removal; fully unused arrays are deleted.
